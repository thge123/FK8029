...
from numpy import zeros,copy,matmul,eye,dot,array,nan,isnan,exp
...
def main():
    ...
    A = getA(t,K,ell=0,beta=Beta)
    B = getB(t,K)

    x0 = rand(len(A))
    eig = InvPower(A,B,x0,shift=-100)
    print(eig)
    ...
    u_excSqrd = lambda x: u_exc(x)**2/B
    plt.plot(xx,u_excSqrd(xx))
